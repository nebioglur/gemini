    ...
    try:
        ...
        try:
            ...
            try:
                ...
                import io
                tablolar = pd.read_html(io.StringIO(html_str))
                for i, df in enumerate(tablolar):
                    if df.shape[1] < 5:
                        continue
                    df["sayfa"] = f"gun_{i+1}"
        ...
        try:
            ...
            if not df_sin.empty:
                def raw_rasat_olustur(row):
                    items = []
                    # Metadata hariç tüm sütunları al
                    exclude = ['sayfa', 'gmt', 'tarih', 'istasyon_no', 'personel', 'g924', 'hadise_kayit']
                    for col in df_sin.columns:
                            ...
                            if pd.notna(val):
                                ...
                                if val_str:
                                    ...
                                    if "UNNAMED" in col_name:
                                        ...
                                        items.append(f"{col_name}:{val_str}")
                    return " ".join(items)
            ...
            def hata_ara(s):
                ...
                if gmt == 6 and (pd.isna(e) or pd.isna(tg)):
                    kodlar.append("h196")
            ...
            # Tarihe göre sırala
            try:
                birlesik["_sort_date"] = pd.to_datetime(birlesik["Tarih"], format="%d.%m.%Y", errors='coerce')
                birlesik = birlesik.sort_values(by=["_sort_date", "gmt"])
                birlesik = birlesik.drop(columns=["_sort_date"])
            except: pass

            col_map = {
                'gmt': 'Saat (GMT)', 'ir': 'İndikatör (ir)', 'ix': 'İndikatör (ix)',
            ...
            cikti = os.path.join(masaustu, f"DENETIM_{yil}_{ay:02d}.xlsx")
...
def get_default_btn_text():
    simdi_btn = datetime.datetime.now()
    v_ay = simdi_btn.month - 1 if simdi_btn.month > 1 else 12
    v_yil = simdi_btn.year if simdi_btn.month > 1 else simdi_btn.year - 1
    return f"RAPOR OLUŞTUR ({v_ay:02d}/{v_yil})"
